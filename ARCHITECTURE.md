...
2. 玩家点击 `Open Poker App`、通过 `/poker_launch` 打开 Activity，或用 `/poker_open table_id` 在手机端进入指定桌。
3. Activity 打开 `/` lobby，前端用 Embedded App SDK 自动认证；浏览器模式可走 `/login` fallback。
4. lobby 轮询 `/api/lobby` 显示 live tables；如果启动前记录了目标桌，授权后自动跳到该桌。
...
- 空桌超过 10 分钟由 `TableRegistry.prune_empty_tables()` 自动移除。
